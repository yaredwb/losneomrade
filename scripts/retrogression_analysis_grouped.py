...
def main():
    parser = argparse.ArgumentParser(description="Run Retrogression Analysis (Grouped Parallel)")
    ...
    print("\n" + "="*70)
    print("RETROGRESSION ANALYSIS (GROUPED PARALLEL)")
    print("="*70)
    ...
    print(f"  - Max retrogression distance: {MAX_LENGTH}m (for grouping)")
    ...
    # 4. Run Retrogression Analysis (GROUPED PARALLEL)
    print(f"\nRunning retrogression analysis (Grouped Parallel)...")
    start_time = time.time()
    
    try:
        retro_result = retrogression.run_retrogression_parallel_grouped(
            bounds=bounds,
    ...
    output_file = os.path.join(args.output_dir, "retrogression_analysis_grouped.geojson")
